...
function Slider({ slides }) {
  ...
  return (
    <div className="overflow-hidden relative mt-4">
      <div
        className={`flex transition ease-out duration-400 rounded-2xl`}
      ...
      <div className="flex justify-center mt-3">
        <div className="bg-gray-200 flex py-1 px-2 rounded-3xl gap-1">
          {slides.map((img, index) => (
            <button
              className={` h-2 w-2 rounded-full hover:cursor-pointer ${
                index === current ? "bg-gray-500" : "bg-gray-400"
              }`}
              key={index}
              onClick={() => {
                setCurrent(index);
